...
def ID3(examples, default):
  ...
  if flag == 0:
    ...
    if 'X50k.year' in attributes:
      attributes.remove('X50k.year')
    flag = 1
...
def check_same_class(examples):
  ...
  '''
  first_ex_value = examples[0]['X50k.year']
  for e in examples:
    if e['X50k.year'] != first_ex_value:
      return None
...
def mode(node, examples):
  ...
  count = {}
  for e in examples:
    if e['X50k.year'] in count:
      count[e['X50k.year']] += 1.0
    else:
      count[e['X50k.year']] = 1.0
